...
class EDHFlow:
    ...
    @tf.function
    def _flow_update(self, observation: tf.Tensor, particles: tf.Tensor,
        ...
        for k in tf.range(num_flow_steps):
            ...
            A, b = self.compute_flow_parameters(current_particles, observation, lam,
    ...
    def filter_with_ukf(self, observations: tf.Tensor, num_flow_steps: int = 10,step_sizes:tf.Tensor=None,resample:bool=False) -> tuple[tf.Tensor, tf.Tensor, tf.Tensor]:
        ...
        for t in tf.range(T):
            ...
            Y_t = Y_time_major[t]
            particles, x_pred_ukf, P_pred_ukf = tf.cond(
                t > 0,
                lambda: (
                    self._transition(particles),
                    *self.ukf.predict_step(x_filt, P_ukf, Wm, Wc, lam_ukf, Q, q_mean)
                ),
...
class LEDHFlow(EDHFlow):
    ...
    def compute_flow_parameters(self, particles: tf.Tensor, observation: tf.Tensor, lam: float,
        ...
        S = R_tiled + lam * HPHt
        eye_obs = tf.eye(obs_dim, batch_shape=[B, N])
        S = 0.5 * (S + tf.linalg.matrix_transpose(S)) + 1e-3 * eye_obs
    ...
    def _flow_update(self, observation: tf.Tensor, particles: tf.Tensor, linearization_points:tf.Tensor=None,
        ...
        if P_xx is None:
            ...
            mean_x = tf.reduce_mean(particles, axis=1, keepdims=True)
            dx = particles - mean_x
            P_xx = tf.matmul(dx, dx, transpose_a=True) / (tf.cast(tf.shape(particles)[1], tf.float32) - 1)
...
class ParticleUKF(UnscentedKalmanFilter):
    ...
    def predict_step(self, x_curr, P_curr, Wm, Wc, lam, Q, noise_proc_mean):
        ...
        if x_curr.shape.ndims == 3:
            ...
            P_flat=0.5*(P_flat + tf.linalg.matrix_transpose(P_flat)) + 1e-3 * tf.eye(D, batch_shape=[B*N])
            ...
            x_pred_flat, P_pred_flat = super().predict_step(
            ...
            return super().predict_step(x_curr, P_curr, Wm, Wc, lam, Q, noise_proc_mean)
    ...
    def update_step(self, x_pred, P_pred, y, Wm, Wc, lam, R, noise_obs_mean):
        ...
        if x_pred.shape.ndims == 3:
            ...
            x_new_flat, P_new_flat, log_l_flat = super().update_step(
                x_flat, P_flat, y_flat, Wm, Wc, lam, R_arg, noise_obs_mean
            )

            x_new = tf.reshape(x_new_flat, [B, N, D])
            P_new = tf.reshape(P_new_flat, [B, N, D, D])
            log_l = tf.reshape(log_l_flat, [B, N])

            return x_new, P_new, log_l

        else:
            return super().update_step(x_pred, P_pred, y, Wm, Wc, lam, R, noise_obs_mean)
...
class InvertiblePFPF(ParticleFilter):
    ...
    def _flow_with_det(self, eta_0, Y_t, num_flow_steps, P_guide=None, eta_mean=None,step_sizes:tf.Tensor=None) -> tuple[tf.Tensor, tf.Tensor]:
        ...
        eta_0_mean=tf.expand_dims(eta_0_mean, -1) # [B, D,1]
        eta_mean=tf.expand_dims(eta_mean, -1) # [B, D,1]
        ...
        for k in tf.range(num_flow_steps):
            ...
            A, b = self.flow_algo.compute_flow_parameters(
                current_particles, Y_t, lam, P_xx=P_guide, linearization_points=eta_mean, eta_0_mean=eta_0_mean
            )

            drift_mean = tf.matmul(A, eta_mean)+ tf.expand_dims(b,-1)  # [B, D, 1] or [B, N, D, 1]
    ...
    def filter(self, observations: tf.Tensor, num_flow_steps: int = 10,step_sizes:tf.Tensor=None) -> tuple[tf.Tensor, tf.Tensor, tf.Tensor, tf.Tensor]:
        ...
        for t in tf.range(T):
            ...
            if t > 0:
                ...
                eta_mean=self.model.transition_fn(pre_imag,tf.zeros_like(pre_imag))
            else:
...
if __name__ == '__main__':
    ...
    t0 = tf.timestamp()
    x_filt, P_filt, all_particles = pf.filter_with_ukf(y_obs, num_flow_steps=2)
    t1 = tf.timestamp()
    print(f"EDH Flow completed in {t1 - t0:.2f} seconds.")

    t2 = tf.timestamp()
    x_filt_ledh, P_filt_ledh, all_particles_ledh = lpf.filter_with_ukf(y_obs, num_flow_steps=2)
    t3 = tf.timestamp()
    print(f"LEDH Flow completed in {t3 - t2:.2f} seconds.")

    t4 = tf.timestamp()
    x_filt_pfpf, P_filt_pfpf, all_particles_pfpf, all_weights_pfpf = pfpf.filter(y_obs, num_flow_steps=2)
    t5 = tf.timestamp()
    print(f"Particle Flow Particle Filter completed in {t5 - t4:.2f} seconds.")
    print(x_filt_pfpf.shape)

    # Plot results
    plt.figure(figsize=(12, 6))
    plt.plot(x_true.numpy(), label='True State', color='g')
    plt.plot(x_filt[0, :, 0].numpy(), label='EDH Filtered State', color='b')
    plt.plot(x_filt_ledh[0, :, 0].numpy(), label='LEDH Filtered State', color='y', linestyle='--')
    plt.plot(x_filt_pfpf[0, :, 0].numpy(), label='PF-PF (EDH) Filtered State', color='m', linestyle='--')
    plt.fill_between(range(T),
                     x_filt_pfpf[0, :, 0].numpy() - tf.sqrt(P_filt_pfpf[0, :, 0, 0]).numpy(),
                     x_filt_pfpf[0, :, 0].numpy() + tf.sqrt(P_filt_pfpf[0, :, 0, 0]).numpy(),
