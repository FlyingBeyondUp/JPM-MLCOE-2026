...
class NLSSM:
    def __init__(self,state_dim:int,obs_dim:int,
                 ...
                 x0: tf.Tensor,
                 get_observation_dist:Union[None,Callable[[tf.Tensor],tfd.Distribution]]=None,
                 get_transition_dist:Union[None,Callable[[tf.Tensor],tfd.Distribution]]=None):
        '''
        ...
        self.get_observation_dist = get_observation_dist
        self.get_transition_dist = get_transition_dist
    ...
    def batch_sample(self, T: int, batch_size: int) -> tuple[tf.Tensor, tf.Tensor]:
        x_ta = tf.TensorArray(dtype=tf.float32, size=T, element_shape=[batch_size, self.state_dim])
        y_ta = tf.TensorArray(dtype=tf.float32, size=T, element_shape=[batch_size, self.obs_dim])

        init_noise_sample = self.init_noise.sample(batch_size)
        # ensure noise shape: [Batch, State_Dim]
        if len(init_noise_sample.shape) == 1:
            init_noise_sample = tf.expand_dims(init_noise_sample, axis=-1)

        # Broadcast x0: [State_Dim] -> [1, State_Dim] -> [Batch, State_Dim]
        x_t = self.x0 + init_noise_sample
        for t in tf.range(T):
            proc_noise = self.process_noise.sample(batch_size)
            if len(proc_noise.shape) == 1:
                proc_noise = tf.expand_dims(proc_noise, axis=-1)
            x_t = tf.cond(
                t > 0,
                lambda: self.transition_fn(x_t, proc_noise),  # x_t is [Batch, Dim], works naturally!
                lambda: x_t  # At t=0, we keep the initialized x_0
            )
            obs_noise = self.observation_noise.sample(batch_size)
            if len(obs_noise.shape) == 1:
                obs_noise = tf.expand_dims(obs_noise, axis=-1)
            y_t = self.observation_fn(x_t, obs_noise)
            x_ta = x_ta.write(t, x_t)
            y_ta = y_ta.write(t, y_t)

        # shapes: [T, Batch, Dim] -> transpose to [Batch, T, Dim]
        x_out = tf.transpose(x_ta.stack(), perm=[1, 0, 2])
        y_out = tf.transpose(y_ta.stack(), perm=[1, 0, 2])
        return x_out, y_out
...
def get1DStochasticVolModel(alpha:float,sigma:float,beta:float,heavy_tail:bool=False)->NLSSM:
    '''
    ...
    def get_observation_dist(x:tf.Tensor)->tfd.Distribution:
        scale = beta * tf.exp(x / 2.0)
    ...
    return NLSSM(state_dim, obs_dim,
                 ...
                 init_noise_dist,
                 x0,get_observation_dist=get_observation_dist)
...
def get_LogSVM_LGSSM(alpha: float, sigma: float, beta: float) -> tuple[LGSSM, tf.Tensor]:
    ...
    R = tf.constant([[math.pi**2 / 2]], dtype=tf.float32)  # The variance of log(chi^2_1) is exactly pi^2 / 2 (~4.93)
...
def getVasicekBondPriceModel(kappa: float, theta: float, sigma: float, tau: float, dt: float,x0:tf.Tensor=tf.zeros((1,))) -> NLSSM:
    ...
    obs_noise_std = 0.02  # e.g., 2 cents on a $1 par value
...
class ExtendedKalmanFilter:
    ...
    def filter(self, y: tf.Tensor, T: int, requires_stabilization=True, return_jacobians=False):
        """
        Batched EKF Filter.
        y: [Batch, T, Obs_Dim] OR [T, Obs_Dim]
        """
        # 1. Handle Batch Dimension
        # If input is [T, Obs], expand to [1, T, Obs]
        rank = len(y.shape)
        if rank == 2:
            is_batched = False
            ...
            raise ValueError(f"y must be rank 2 or 3, got {rank}")

        batch_size = tf.shape(y_in)[0]
        state_dim = self.model.state_dim
        obs_dim = self.model.obs_dim

        # Expansion: [1, Dim, Dim] to broadcast against [Batch, Dim, Dim]
        Q = tf.expand_dims(self._get_cov(self.model.process_noise), 0)
        R = tf.expand_dims(self._get_cov(self.model.observation_noise), 0)
        P0 = tf.expand_dims(self._get_cov(self.model.init_noise), 0)

        # x0: [Dim] -> [1, Dim, 1] -> Tile to [Batch, Dim, 1]
        x0_reshaped = tf.reshape(self.model.x0, [1, state_dim, 1])
        x_curr = tf.tile(x0_reshaped, [batch_size, 1, 1])

        # P0: [1, Dim, Dim] -> Tile to [Batch, Dim, Dim]
        P_curr = tf.tile(P0, [batch_size, 1, 1])

        # Zero noise for linearization [Batch, Dim]
        zero_proc = tf.zeros([batch_size, state_dim], dtype=tf.float32)
        zero_obs = tf.zeros([batch_size, obs_dim], dtype=tf.float32)
        ...
        for t in range(T):
            if t == 0:
                x_pred = x_curr
                P_pred = P_curr
                # Padding A for t=0 (not used but keeps shapes consistent)
                A_pad = tf.eye(state_dim, batch_shape=[batch_size])
                A_arr = A_arr.write(t, A_pad)  # or skip writing
            else:
                x_flat = tf.squeeze(x_curr, -1)
                f_val, A, W = self._batch_linearize(self.model.transition_fn, x_flat, zero_proc)
                # Reshape Output to [Batch, Dim, 1]
                x_pred = tf.expand_dims(f_val, -1)

                # A: [Batch, Dim, Dim], W: [Batch, Dim, Dim]
                # P_pred = A P A^T + W Q W^T
                P_pred = A @ P_curr @ tf.transpose(A, perm=[0, 2, 1]) + \
                         W @ Q @ tf.transpose(W, perm=[0, 2, 1])

                A_arr = A_arr.write(t - 1, A)

            x_pred_flat = tf.squeeze(x_pred, -1)
            h_val, C, V = self._batch_linearize(self.model.observation_fn, x_pred_flat, zero_obs)

            # Innovation: y[t] is [Batch, Obs] -> [Batch, Obs, 1]
            y_t = tf.expand_dims(y_in[:, t, :], -1)
            h_val = tf.expand_dims(h_val, -1)
            innov = y_t - h_val

            # R_eff = V R V^T
            R_eff = V @ R @ tf.transpose(V, perm=[0, 2, 1])

            # S = C P C^T + R_eff
            S_t = C @ P_pred @ tf.transpose(C, perm=[0, 2, 1]) + R_eff + 1e-6 * tf.eye(obs_dim)

            S_chol = tf.linalg.cholesky(S_t)

            PCt = P_pred @ tf.transpose(C, perm=[0, 2, 1])  # [B, State, Obs]
            # K = PCt @ S^-1.
            ...
            if requires_stabilization:
                # Joseph Form
                P_curr = I_KC @ P_pred @ tf.transpose(I_KC, perm=[0, 2, 1]) + \
                         K_t @ R_eff @ tf.transpose(K_t, perm=[0, 2, 1])
            else:
                P_curr = I_KC @ P_pred

            P_curr = 0.5 * (P_curr + tf.transpose(P_curr, perm=[0, 2, 1]))

            # Log Likelihood
            log_det_S = 2.0 * tf.reduce_sum(tf.math.log(tf.linalg.diag_part(S_chol)), axis=-1)
            # quad = innov^T S^-1 innov
            # z = S^-1 innov
            z = tf.linalg.cholesky_solve(S_chol, innov)
            quad_term = tf.squeeze(tf.transpose(innov, perm=[0, 2, 1]) @ z, axis=[-2, -1])
        ...
        # [T, Batch, Dim, 1] -> [Batch, T, Dim, 1]
        x_filt = tf.transpose(x_filt_arr.stack(), perm=[1, 0, 2, 3])
        P_filt = tf.transpose(P_filt_arr.stack(), perm=[1, 0, 2, 3])
        x_pred = tf.transpose(x_pred_arr.stack(), perm=[1, 0, 2, 3])
        P_pred = tf.transpose(P_pred_arr.stack(), perm=[1, 0, 2, 3])

        x_filt = tf.squeeze(x_filt, -1)
        x_pred = tf.squeeze(x_pred, -1)
        results = (x_filt, P_filt, x_pred, P_pred, log_l)

        if not is_batched:
            # Squeeze batch dim if input was single
            results = (x_filt[0], P_filt[0], x_pred[0], P_pred[0], log_l[0])

        if return_jacobians:
            # Handle A_arr similarly
            A_stack = tf.transpose(A_arr.stack(), perm=[1, 0, 2, 3])
            if not is_batched: A_stack = A_stack[0]
            return results + (A_stack,)
    ...
    def smooth(self, y: tf.Tensor, T: int):
        ...
        for t in tf.range(T - 2, -1, -1):
            ...
            x_filt_t = tf.reshape(x_filt[t], [self.model.state_dim, 1])
            x_pred_next = tf.reshape(x_pred[t + 1], [self.model.state_dim, 1])
            x_next_smooth = tf.reshape(x_next_smooth, [self.model.state_dim, 1])

            x_curr = x_filt_t + J_t @ (x_next_smooth - x_pred_next)
            ...
            x_smooth = x_smooth.write(t, tf.reshape(x_curr, [-1]))
...
class UnscentedKalmanFilter(tf.Module):
    ...
    def _generate_sigma_points(self, x, P, lam):
        ...
        P_sym = 0.5 * (P + tf.linalg.matrix_transpose(P)) + 1e-3 * tf.eye(n)
    ...
    def _compute_stats(self, sigma_points, Wm, Wc, noise_cov):
        ...
        P = 0.5 * (P + tf.linalg.matrix_transpose(P))+1e-3 * tf.eye(tf.shape(P)[-1], batch_shape=[tf.shape(P)[0]])
    ...
    def predict_step(self, x_curr, P_curr, Wm, Wc, lam, Q, noise_proc_mean):
        ...
        # [Batch_Total, Dim]
        sig_pts_pred_flat = self.model.transition_fn(sig_pts_flat, noise_proc_mean)
        sig_pts_pred = tf.reshape(sig_pts_pred_flat, [batch_size, num_sig, self.state_dim])

        x_pred, P_pred, _ = self._compute_stats(sig_pts_pred, Wm, Wc, Q)
    ...
    def update_step(self, x_pred, P_pred, y, Wm, Wc, lam, R, noise_obs_mean):
        ...
        #S_chol = tf.linalg.cholesky(S + 1e-3 * tf.eye(self.obs_dim))

        # Solve S * K^T = P_xy^T  => K^T = S^-1 P_xy^T
        # Input  [Batch, Obs, State]
        Kt_transposed = tf.linalg.solve(S, tf.linalg.matrix_transpose(P_xy))
        ...
        _,log_det_S= tf.linalg.slogdet(S)
        sol = tf.linalg.solve(S, innovation_expanded)
    ...
    def filter(self, y: tf.Tensor, py_loop=False):
        ...
        x_init, P_init, Wm, Wc, lam, Q, R, q_mean, r_mean= self.initialize_filter(batch_size)
        ...
        def scan_step(carry, y_t):
            ...
            x_pred, P_pred= self.predict_step(x_prev, P_prev, Wm, Wc, lam, Q, q_mean)
        ...
        def train_step():
            ...
            with tf.GradientTape() as tape:
                ...
                if self.proc_log_scale is not None:
                    min_log_scale = tf.math.log(1e-3)
                    reg_loss += tf.reduce_sum(tf.nn.relu(min_log_scale - self.proc_log_scale))
                if self.obs_log_scale is not None:
                    min_log_scale = tf.math.log(1e-3)
                    reg_loss += tf.reduce_sum(tf.nn.relu(min_log_scale - self.obs_log_scale))
...
def run_example_SVM(alpha, sigma, beta, T=500):
    ...
    # y_log_sq shape: [T, 1]
    y_log_sq = tf.reshape(y_log_sq, [-1, 1])  # [T] -> [T, 1]
    ...
    y_log_sq_bias_corrected = y_log_sq - bias  # 已经是 [T, 1]
    x_filt_kf, P_filt_kf, _, _, log_l_kf = kf.filter(y_log_sq_bias_corrected, T,
                                                     requires_stabilization=True)
    x_filt_kf = tf.squeeze(x_filt_kf, axis=-1)

    # UKF: Batch, T, Obs] -> [1, T, 1]
    y_log_sq_batch = tf.expand_dims(y_log_sq, 0)  # [T, 1] -> [1, T, 1]

    ukf = UnscentedKalmanFilter(
        get1DLogSquaredSVM(alpha=alpha, sigma=sigma, beta=beta),
        alpha=1,
        beta=2.0,
        kappa=0.0
    )
    ...
    t0 = time.time()
    x_filt_ukf, P_filt_ukf, _ = ukf.filter(y_log_sq_batch)
    t1 = time.time()

    x_filt_ukf_py, _, _ = ukf.filter(y_log_sq_batch, py_loop=True)
    t2 = time.time()
    ...
    plt.plot(x_filt_ukf, label='Hidden States from Unscented Kalman Filter', c='orange')
    ...
    plt.ylabel('x_t')
    plt.legend()
    plt.savefig('Figures\KF_UKF_vs_EKF_SVM.pdf', bbox_inches='tight')
...
def run_experiment_Vasicek(kappa, theta, sigma, tau, dt, T=200,x0=tf.zeros((1,))):
    ...
    x_filt_kf, P_filt_kf, _, _, log_l_kf = kf.filter(y_bias_corrected, T,
                                                     requires_stabilization=True)
    x_filt_kf = tf.squeeze(x_filt_kf, axis=-1)

    ukf= UnscentedKalmanFilter(model_vasicek,alpha=1, beta=2, kappa=1.0) # For 1D, alpha=1 is better than 1e-3
...
def run_experiment_Vasicek_EM(kappa, theta, sigma, tau, dt, T=50, batch_size=20, n_em_iter=20):
    ...
    t0=time.time()
    log_likelihoods_ukf = ukf.fit(y_batch, n_iter=10*n_em_iter, learning_rate=0.005)
    t1=time.time()
    ...
    plt.title('UKF Log-Likelihood Progression')
...
if __name__ == '__main__':
    ...
    #run_experiment_Vasicek(kappa=0.2, theta=0.05, sigma=0.04, tau=10.0, dt=1/252,x0=tf.constant((0.06,)))
